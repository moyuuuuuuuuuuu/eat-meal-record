...
class RecommendationBusiness extends BaseBusiness
{
    ...
    public function today(Request $request): array
    {
        ...
        // 已登录：根据近期营养摄入推荐
        return $this->recommendByUserContext($request, $request->userInfo->id);
    }
    ...
    public function recommendByTime(Request $request): array
    {
        ...
        $query = FoodModel::whereHas('tags', function ($query) use ($tagName) {
        ...
        if (!$food) {
            $food = FoodModel::whereHas('tags', function ($query) use ($tagName) {
                $query->where('name', $tagName);
            })->where('status', NormalStatus::YES->value)->inRandomOrder()->first();
        }

        // 最后的兜底：随机给一个非食材分类的
        if (!$food) {
            $food = FoodModel::whereNotIn('cat_id', $rawMaterialCatIds)
                ->where('status', NormalStatus::YES->value)
                ->inRandomOrder()
                ->first();
        }
        ...
        if (!$food) {
            throw new BusinessException('暂无食物推荐', BusinessCode::BUSINESS_ERROR->value);
        }

        $data                     = (new RecommendationFormat($request))->format($food);
    ...
    public function recommendByUserContext(Request $request, int $userId): array
    {
        $config = config('recommendation');
        // 获取最近3天的记录
        $recentRecords = MealRecordModel::where('user_id', $userId)
        ...
        $query = FoodModel::where('status', 1)
            ->whereNotIn('cat_id', $rawMaterialCatIds);
